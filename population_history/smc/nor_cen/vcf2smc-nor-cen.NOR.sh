...
mkdir -p mkdir -p smcpp_results/nor-cen/
mkdir -p smcpp_results/nor-cen/smcformat_no_mask
...
do
    ...
    vcf2smc \
    -d "${i}" "${i}" \
    ${vcf} smcpp_results/nor-cen/smcformat_no_mask/nor-"${i}".NC_0376${SLURM_ARRAY_TASK_ID}.1.smc.gz NC_0376${SLURM_ARRAY_TASK_ID}.1 \
    nor:${northern} 
done 
